/* Capital Higher or Lower — Game Logic */
...
async function init() {
  ...
  document.getElementById('best').textContent = GeoUtils.getHighScore('capital-higher-lower');
  startGame();
}
...
function renderRound() {
  ...
  document.getElementById('capital-known').textContent = knownCountry.capital;
  document.getElementById('nation-known').textContent = knownCountry.name;
  document.getElementById('pop-known').textContent = GeoUtils.formatNumber(knownCountry.capital_population);
  ...
  document.getElementById('capital-unknown').textContent = unknownCountry.capital;
  document.getElementById('nation-unknown').textContent = unknownCountry.name;
...
function guess(direction) {
  ...
  const isHigher = unknownCountry.capital_population >= knownCountry.capital_population;
  const correct = (direction === 'higher' && isHigher) || (direction === 'lower' && !isHigher);

  document.getElementById('pop-unknown').textContent = GeoUtils.formatNumber(unknownCountry.capital_population);
  const unknownCard = document.getElementById('country-unknown');
...
function endGame() {
  ...
  const isNew = GeoUtils.setHighScore('capital-higher-lower', score);
  document.getElementById('best').textContent = GeoUtils.getHighScore('capital-higher-lower');
...
function shareScore() {
  GeoUtils.share('Capital Higher or Lower', score, 'point streak!');
}
